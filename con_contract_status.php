...
class Main {
		...
		$sql = "select * from pdct_contract_status";
		...
		while ($row = $exsql->FetchRow()) {
			$list[] = array("id"=>$row['id'],"contract_status"=>$row['contract_status']);
		...
		$smarty->display("contract_status_opt_view.tpl");
		}
		...
		$smarty->assign("title",$title); 
	 	...
	 	$smarty->assign("level_user",$_SESSION['level_user']);
	 	$smarty->assign("nama_pegawai",$_SESSION['nama_pegawai']);
		$smarty->display("contract_status_opt_add.tpl");			
		}
		...
		$sql = "insert into pdct_contract_status (id,contract_status,user_add,date_add) 
		values
		(null,'$contract_status','$_SESSION[username]',now())";
	 ...
	 //exit();
		...
		if($exsql){
			  echo "<script language=javascript>
	                    ...
	                    window.location='master_contract_status.php';
	                 </script>";
			}else{
				  echo "<script language=javascript>
